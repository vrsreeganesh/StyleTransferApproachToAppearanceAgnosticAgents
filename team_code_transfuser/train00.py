...
class Engine(object):
    ...
    def load_data_compute_loss(self, data):
        ...
        # gridimage = torchvision.utils.make_grid(output, nrow= 1, padding = 2);
        colourmixing = [var00.item() for var00 in torch.randperm(3)];
        print("colourmixing = ", colourmixing)
        gridimage = torchvision.utils.make_grid(output[:, colourmixing, :,:], nrow= 1, padding = 2); # changing the colours
        torchvision.utils.save_image(gridimage, "/projectnb/rlvn/students/vrs/transfuser/RLVNFinalSubmission/STCarla/whole.png");
        # for i in range(output.shape[0]):
        #     var00 = output[i:i+1,:,:,:]
        #     # var00 = output[i,:,:,:]
        #     print("var00.shape = ", var00.shape)
        #     torchvision.utils.save_image(var00, "/projectnb/rlvn/students/vrs/transfuser/RLVNFinalSubmission/STCarla/"+str(i)+"_"+"contentweight="+str(content_weight_arg)+".png");
