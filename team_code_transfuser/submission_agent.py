        ...
        with torch.no_grad():
            ...
            for i in range(self.model_count):
                ...
                if (self.backbone == 'transFuser'):
                    print("------------- line 297 in submission_agent.py --------------")
                    # image, lidar_bev_target_point, target_point_image, velocity, pred_wp ===> all tensors
                    # image.shape = [1,3,160, 704]
                    # lidar_bev.shape = [1,2,256,256]
                    # target_point.shape = [1,2]
                    # target_point_image.shape = [1, 1, 256, 256]
                    # velocity.shape = [1,1]
                    # pred_wp.shape = [1, 4, 2]
                    ...
                    savelocation = "/projectnb/rlvn/students/vrs/transfuser/"
                    torch.save(image, savelocation + "SavedImages/image_" + str(datetime.datetime.now().strftime('%H_%M_%S_%f')[:-3]))
                    print(".", end = "")
                    torch.save(pred_wp, savelocation + "SavedPredictedWaypoints/predwp_" + str(datetime.datetime.now().strftime('%H_%M_%S_%f')[:-3]))
                    # print("pred_wp.shape = ", pred_wp.shape)
                    ...
                    print("------------- latentTF: line 331 in submission_agent.py --------------")
                    # print("type(image) = ", type(image))
                    # print("type(lidar_bev) = ", type(lidar_bev))
                    # print("type(target_point) = ", type(target_point))
                    # print("type(target_point_image) = ", type(target_point_image))
                    # print("type(velocity) = ", type(velocity))
                    pred_wp, rotated_bb = self.nets[i].forward_ego(image, lidar_bev, target_point, 
                                                                   target_point_image, velocity, num_points=num_points)
                    ...
                    # saving stuff
                    savelocation = "/projectnb/rlvn/students/vrs/transfuser/SavedStuff/"
                    currenttime = str(datetime.datetime.now().strftime('%H_%M_%S_%f')[:-3])
                    
                    torch.save(image, savelocation + "image_" + currenttime)
                    torch.save(lidar_bev, savelocation + "lidar_bev_" + currenttime)
                    torch.save(target_point, savelocation + "target_point_" + currenttime)
                    torch.save(target_point_image, savelocation + "target_point_image_" + currenttime)
                    torch.save(velocity, savelocation + "velocity_" + currenttime)
                    torch.save(num_points, savelocation + "num_points_" + currenttime)
                    torch.save(pred_wp, savelocation + "predwp_" + currenttime)
                    torch.save(rotated_bb, savelocation + "rotated_bb_" + currenttime)
