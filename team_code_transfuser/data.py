...
class CARLA_Data(Dataset):
    ...
    def __init__(self, root, config, shared_dict=None):
        ...
        self.seq_len = np.array(config.seq_len) # The number of time-steps that are provided as input. Since driving is generally markovian,chances are this is going to be default one. 
        assert (config.img_seq_len == 1) # making sure that for images, only that of one time step is taken
        self.pred_len = np.array(config.pred_len) # number of future waypoints that are predicted. Default is 4

        self.img_resolution = np.array(config.img_resolution) # image resolution, [160,704]
        self.img_width = np.array(config.img_width) # the image width is given as 320 in config.py
        self.scale = np.array(config.scale) # how much we'll be scaling the input image. Its given as one for now. 
        self.multitask = np.array(config.multitask) # whether to use segmentation + depth losses
